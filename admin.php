...
if (isset($_POST['login_admin'])) {
    ...
    // Memastikan pengguna memasukkan username dan password apa saja
    if (!empty($username) && !empty($password)) {
        session_start();

        // Menyimpan username dan password ke dalam sesi
        $_SESSION['username'] = $username;
        $_SESSION['password'] = $password;

        echo "<script>document.location.href='home_admin.php'</script>";
    } else {
        echo "<script>alert('Username dan Password tidak boleh kosong!');document.location.href='admin.php'</script>";
    }
}
